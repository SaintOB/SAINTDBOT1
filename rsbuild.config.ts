...
export default defineConfig({
    ...
    dev: {
        ...
        setupMiddlewares: [
            (middlewares, _server) => {
                // Gated bot file API — server-side validates Deriv token and allowlist
                // before serving private XML/MT5 bot files.
                // eslint-disable-next-line @typescript-eslint/no-var-requires
                const { handleBotRequest, isBotApiRequest } = require('./scripts/bot-api.cjs');
                ...
                // Debug endpoint to receive full error details from browser
                middlewares.unshift((req: any, res: any, next: any) => {
                    if (req.method === 'POST' && req.url === '/__debug_error') {
                        let body = '';
                        ...
                        req.on('end', () => {
                            try {
                                ...
                                console.error('[DEBUG-BROWSER]', logLine);
                                fs.appendFileSync('/tmp/browser_debug_errors.log', logLine);
                            } catch (e) {
                                console.error('[DEBUG-BROWSER] Parse error:', e);
                            }
