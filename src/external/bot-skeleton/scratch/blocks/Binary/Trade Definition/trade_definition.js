...
window.Blockly.JavaScript.javascriptGenerator.forBlock.trade_definition = block => {
    ...
    // For Even/Odd, always subscribe to BOTH proposals (DIGITEVEN + DIGITODD) so the
    // Purchase block can freely choose either side without a "no proposal" error.
    const contract_type_list =
        contract_type === 'both' || trade_type === 'evenodd'
