...
window.Blockly.Blocks.purchase = {
    ...
    populatePurchaseList(event) {
        ...
        if (trade_definition_block) {
            ...
            // For Even/Odd, always expose BOTH options in the Purchase dropdown so the user
            // can freely pick EVEN or ODD, regardless of what the Contract Type field is set to.
            const effective_contract_type = trade_type === 'evenodd' ? 'both' : contract_type;
            const contract_type_options = getContractTypeOptions(effective_contract_type, trade_type);
