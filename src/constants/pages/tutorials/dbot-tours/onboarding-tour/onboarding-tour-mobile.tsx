...
const OnboardingTourMobile = observer(() => {
    ...
    React.useEffect(() => {
        const checkTokenForTour = () => {
            ...
            if (!token && active_tab === 0) {
                // Skip the mobile onboarding tour entirely — the slider's
                // Skip / Start buttons get clipped by Safari's bottom toolbar
                // + the fixed Run panel on iPhone, leaving the user with no
                // way to dismiss it. Auto-mark the tour as seen.
                storeSetting('onboard_tour_token', new Date().getTime());
                return;
            }
        };
        checkTokenForTour();
    ...
    // Hard-disable the mobile onboarding slider on touch devices
    return null;
