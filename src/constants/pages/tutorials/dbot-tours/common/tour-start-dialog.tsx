...
const TourStartDialog = observer(() => {
    ...
    React.useEffect(() => {
        ...
        if (is_tour_dialog_visible) {
            // On mobile the Skip / Start buttons get pushed off-screen by the
            // Safari bottom toolbar + the disclaimer banner — the dialog has
            // `has_close_icon={false}` so the user is left with no way to
            // dismiss. Auto-skip on mobile to prevent the freeze.
            if (!isDesktop) {
                setIsTourOpen(false);
                if (typeof setShowMobileTourDialog === 'function') setShowMobileTourDialog(false);
                setTourDialogVisibility(false);
                setActiveTour('');
                setTourSettings(new Date().getTime(), tour_token);
                return;
    ...
    }, [is_tnc_needed, is_tour_dialog_visible, isDesktop]);
