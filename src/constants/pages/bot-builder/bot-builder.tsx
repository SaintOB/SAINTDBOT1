...
const BotBuilder = observer(() => {
    ...
    React.useEffect(() => {
        ...
        // These block types hold trade parameters — visitors may edit their values
        const PARAMETER_BLOCK_TYPES = ['math_number', 'math_number_positive', 'trade_definition_market', 'purchase'];

        const lockAllBlocks = (workspace: any) => {
            if (!workspace) return;
            workspace.getAllBlocks(false).forEach((block: any) => {
                block.setMovable(false);
                block.setDeletable(false);
                // Allow editing of numeric parameter blocks only
                if (!PARAMETER_BLOCK_TYPES.includes(block.type)) {
                    block.setEditable(false);
                }
            });
            // Hide toolbox so visitors cannot add new blocks
            const toolboxDiv = workspace.getToolbox?.()?.HtmlDiv;
            if (toolboxDiv) toolboxDiv.style.display = 'none';
        };

        const preventEdits = (event: any) => {
            const ws = window.Blockly?.derivWorkspace;
            if (!ws) return;
            // Re-lock any newly created or moved block
            if (
        ...
        const ws = window.Blockly?.derivWorkspace;
        lockAllBlocks(ws);
        if (ws) ws.addChangeListener(preventEdits);

        const timer = setTimeout(() => {
            const wsDelayed = window.Blockly?.derivWorkspace;
            lockAllBlocks(wsDelayed);
        }, 800);

        return () => {
            clearTimeout(timer);
            window.Blockly?.derivWorkspace?.removeChangeListener(preventEdits);
        };
    }, [isOwner, is_loading]);
