...
const AppWrapper = observer(() => {
    ...
    const hash = ['dashboard', 'bot-builder', 'chart', 'tutorial', 'custom-bots', 'analysis-tool'];
    ...
    const GetHashedValue = (tab: number) => {
        // Support both clean paths (/custom-bots) and legacy hashes (#custom_bots → custom-bots)
        const from_path = location.pathname.replace(/^\//, '');
        const from_hash = location.hash?.split('#')[1]?.replace(/_/g, '-');
        tab_value = from_path || from_hash;
    ...
    React.useEffect(() => {
        ...
        if (init_render.current) {
            ...
            window.history.replaceState({}, '', `/${hash[active_tab] || hash[0]}`);
        }
        if (active_tour !== '') {
            setActiveTour('');
        }
        ...
        if (!isOAuth2Enabled) {
            ...
            try {
                ...
                if (tmbEnabled) {
                    ...
                    // Skip OIDC for custom domains where client_id lookup fails
                                ...
                                label={
                                    <>
                                        <LabelPairedObjectsColumnCaptionRegularIcon
                                            ...
                                            fill='var(--text-general)'
                                        />
                                        <Localize i18n_default_text='Saint Bots' />
                                    </>
                                }
                                id='id-free-bots'
                            >
