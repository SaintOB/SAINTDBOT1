...
#property description "Team.Saintfx HF Scalper Ultra 2026: precision entries, adaptive ATR exits, enhanced risk management."
...
input ulong  InpMagic                        = 750015;
input int    InpMinSecondsBetweenTrades      = 1;
input int    InpMaxOpenPositions             = 1;
input bool   InpUseRiskPercent               = true;
input double InpRiskPercentPerTrade          = 0.40;  // % of balance per trade
...
input group "=== Volatility Regime (ATR M1) ==="
input int    InpATRPeriod                    = 14;
input int    InpATRMinPoints                 = 20;
input int    InpATRMaxPoints                 = 450;
input double InpSL_ATR_Multiplier            = 0.40;
input double InpTP_ATR_Multiplier            = 0.65;
input int    InpMinSLPoints                  = 25;
input int    InpMinTPPoints                  = 35;
input int    InpMaxSLPoints                  = 80;
input int    InpMaxTPPoints                  = 130;
...
input int    InpMicroTakePoints              = 10;
...
void TryOpenTrade()
{
    ...
    if (sig > 0)
    {
        ...
        sent = trade.Buy(lots, S(), tick.ask,
                         NormalizeDouble(sl, digits), NormalizeDouble(tp, digits), "TSFX_PRO");
    }
    else
    {
...
int OnInit()
{
    ...
    ChartIndicatorAdd(0, 0, hFastEMA);
    ChartIndicatorAdd(0, 0, hSlowEMA);
    ChartIndicatorAdd(0, 0, hM15EMA);
    ChartIndicatorAdd(0, 1, hRSI);
    ChartIndicatorAdd(0, 2, hATR);

    string banner = StringFormat(
        "Team.Saintfx HF Scalper Ultra 2026 | %s | Risk: %.2f%% | StreakReduce: after %d losses → %.0f%%",
        S(), InpRiskPercentPerTrade, InpLossStreakReduceAfter,
...
void OnTick()
{
    ...
    string status = StringFormat(
        "Team.Saintfx HF Scalper Ultra 2026 | %s | Trades:%d/%d | DD:%.2f%% | Pause:%s | Risk: %s",
        S(),
        g_trades_today,
        InpMaxTradesPerDay,
